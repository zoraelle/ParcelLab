...
class DInterface(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if stage in (None, "fit"):
            self.train_set = self._instantiate_dataset(self.train_dataset_name, split="train")
            self.val_sets = [self._instantiate_dataset(name, split="val") for name in self.val_dataset_names]
